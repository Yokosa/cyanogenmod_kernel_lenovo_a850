...
u32 pinSet[2][8] = {
    ...
    if (On) {
		...
		{
				PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_A5142_MIPI_RAW \n");
				PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);

				PK_DBG("kdCISModulePowerOn a5142 power on -on:pinSetIdx=%d\n",pinSetIdx);

            //disable inactive sensor
            if(pinSetIdx == 0 || pinSetIdx == 2) {//disable sub
                PK_DBG("kdCISModulePowerOn a5142 disable sub -on:pinSetIdx=%d\n",pinSetIdx);
    ...
    else
    {
	 ...
	 else
	 {
		...
		if((currSensorName && (0 == strcmp(SENSOR_DRVNAME_A5142_MIPI_RAW,currSensorName))))
		{
			PK_DBG("kdCISModulePower--off get in---SENSOR_DRVNAME_A5142_MIPI_RAW \n");
